...
check_3_1() {
  ...
  if [ -f "$file" ]; then
    if [ "$(stat -c %u%g $file)" -eq 00 ]; then
      ...
      warn "$check_3_1"
      note "     * Wrong ownership for $file"
      resulttestjson "WARN" "Wrong ownership for $file"
      currentScore=$((currentScore - 1))
    fi
  else
    info "$check_3_1"
    note "     * Recommendation not applicable: file not found on $file"
...
check_3_2() {
  ...
  if [ -f "$file" ]; then
    if [ "$(stat -c %a $file)" -le 644 ]; then
      ...
      warn "$check_3_2"
      note "     * Wrong permissions for $file"
...
check_3_5() {
  ...
  if [ -d "$directory" ]; then
    if [ "$(stat -c %u%g $directory)" -eq 00 ]; then
      ...
      warn "$check_3_5"
      note "     * Wrong ownership for $directory"
      resulttestjson "WARN" "Wrong ownership for $directory"
      currentScore=$((currentScore - 1))
    fi
  else
    info "$check_3_5"
    note "     * Directory not foundon $directory"
...
check_3_6() {
  ...
  if [ -d "$directory" ]; then
    if [ "$(stat -c %a $directory)" -le 755 ]; then
      ...
      warn "$check_3_6"
      note "     * Wrong permissions for $directory"
...
check_3_9() {
  ...
  if [ -f "$tlscacert" ]; then
    if [ "$(stat -c %u%g "$tlscacert")" -eq 00 ]; then
      ...
      warn "$check_3_9"
      note "     * Wrong ownership for $tlscacert"
      resulttestjson "WARN" "Wrong ownership for $tlscacert"
      currentScore=$((currentScore - 1))
    fi
  else
    info "$check_3_9"
    note "     * No TLS CA certificate found $tlscacert"
...
check_3_10() {
  ...
  if [ -f "$tlscacert" ]; then
    if [ "$(stat -c %a $tlscacert)" -le 444 ]; then
      ...
      warn "$check_3_10"
      note "     * Wrong permissions for $tlscacert"
...
check_3_11() {
  ...
  if [ -f "$tlscert" ]; then
    if [ "$(stat -c %u%g "$tlscert")" -eq 00 ]; then
      ...
      warn "$check_3_11"
      note "     * Wrong ownership for $tlscert"
      resulttestjson "WARN" "Wrong ownership for $tlscert"
      currentScore=$((currentScore - 1))
    fi
  else
    info "$check_3_11"
    note "     * No TLS Server certificate found $tlscert"
...
check_3_12() {
  ...
  if [ -f "$tlscert" ]; then
    if [ "$(stat -c %a $tlscert)" -le 444 ]; then
      ...
      warn "$check_3_12"
      note "     * Wrong permissions for $tlscert"
      resulttestjson "WARN" "Wrong permissions for $tlscert"
      currentScore=$((currentScore - 1))
    fi
  else
    info "$check_3_12"
    note "     * No TLS Server certificate found"
    resulttestjson "INFO" "No TLS Server certificate found $tlscert"
    currentScore=$((currentScore + 0))
  fi
}
...
check_3_13() {
  ...
  if [ -f "$tlskey" ]; then
    if [ "$(stat -c %u%g "$tlskey")" -eq 00 ]; then
      ...
      warn "$check_3_13"
      note "     * Wrong ownership for $tlskey"
      resulttestjson "WARN" "Wrong ownership for $tlskey"
      currentScore=$((currentScore - 1))
    fi
  else
    info "$check_3_13"
    note "     * No TLS Key found $tlskey"
...
check_3_14() {
  ...
  if [ -f "$tlskey" ]; then
    if [ "$(stat -c %a $tlskey)" -eq 400 ]; then
      ...
      warn "$check_3_14"
      note "     * Wrong permissions for $tlskey"
