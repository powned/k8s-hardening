...
check_2_1() {
  ...
  if get_docker_effective_command_line_args '--icc' | grep false >/dev/null 2>&1; then
    ...
    warn "$check_2_1"
    note "     * Run the docker daemon with icc arg: $ dockerd --icc=false"
    note "     * Or edit the configuration file with 'icc': false"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
  fi
}
...
check_2_2() {
  ...
  if get_docker_configuration_file_args 'log-level' >/dev/null 2>&1; then
    if get_docker_configuration_file_args 'log-level' | grep info >/dev/null 2>&1; then
      ...
      warn "$check_2_2"
      note "       * Run the docker daemon with log-level arg: $ dockerd --log-level='info'"
      note "       * Or edit the configuration file with 'log-level': 'info'"
      resulttestjson "WARN"
      currentScore=$((currentScore - 1))
    fi
...
check_2_3() {
  ...
  if get_docker_effective_command_line_args '--iptables' | grep "false" >/dev/null 2>&1; then
    warn "$check_2_3"
    note "       * NOT run the docker daemon with --iptables=false parameter"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
...
check_2_5() {
  ...
  if docker info 2>/dev/null | grep -e "^\sStorage Driver:\s*aufs\s*$" >/dev/null 2>&1; then
    warn "$check_2_5"
    note "       * Do NOT explicit use aufs as storage driver. NOT start with: $ dockerd --storage-driver aufs"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
  else
    pass "$check_2_5"
...
check_2_6() {
  ...
  if [ $(get_docker_configuration_file_args 'tcp://') ] || \
    ...
    info "$check_2_6"
    note "     * Docker daemon not listening on TCP"
...
check_2_7() {
  ...
  if get_docker_configuration_file_args 'default-ulimit' | grep -v '{}' >/dev/null 2>&1; then
    ...
    info "$check_2_7"
    note "     * Default ulimit doesn't appear to be set"
...
check_2_8() {
  ...
  if get_docker_configuration_file_args 'userns-remap' | grep -v '""'; then
    pass "$check_2_8"
    note "     * CAUTION: there is a bug with userns-remap=default. Further information in README.md"
    resulttestjson "PASS"
    currentScore=$((currentScore + 1))
    ...
    warn "$check_2_8"
    note "     * Ensure this files are created: $ sudo touch /etc/subuid /etc/subgid"
    note "     * Start Docker with this parameter: $ dockerd --userns-remap=default"
    note "     * CAUTION: there is a bug with userns-remap=default. Further information in README.md"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
  fi
...
check_2_9() {
  ...
  if get_docker_configuration_file_args 'cgroup-parent' | grep -v ''; then
    warn "$check_2_9"
    note "     * Confirm cgroup usage. To specifically start with: $ dockerd --cgroup-parent=/foobar"
...
check_2_10() {
  ...
  if get_docker_configuration_file_args 'storage-opts' | grep "dm.basesize" >/dev/null 2>&1; then
    warn "$check_2_10"
    note "     * NOT set --storage-opt dm.basesize until needed"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
...
check_2_11() {
  ...
  if get_docker_configuration_file_args 'authorization-plugins' | grep -v '\[]'; then
    ...
    warn "$check_2_11"
    note "     * You should create an authorization plugin"
    note "     * Impact: this may have a performance impact"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
  fi
}
...
check_2_13() {
  ...
  if docker info 2>/dev/null | grep -e "Live Restore Enabled:\s*true\s*" >/dev/null 2>&1; then
    ...
    if docker info 2>/dev/null | grep -e "Swarm:*\sactive\s*" >/dev/null 2>&1; then
      ...
      warn "$check_2_13"
      note "     * Run Docker daemon with: $ dockerd --live-restore"
      resulttestjson "WARN"
      currentScore=$((currentScore - 1))
    fi
  fi
...
check_2_14() {
  ...
  if get_docker_configuration_file_args 'userland-proxy' | grep false >/dev/null 2>&1; then
    ...
    warn "$check_2_14"
    note "     * Run Docker daemon with: $ dockerd --userland-proxy=false"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
  fi
}
...
check_2_16() {
  ...
  # if [ "$docker_version" -le 1903 ]; then
    if docker version -f '{{.Server.Experimental}}' | grep false 2>/dev/null 1>&2; then
      ...
      warn "$check_2_16"
      note "     * You should not pass --experimental as runtime parameter on production systems"
      resulttestjson "WARN"
      currentScore=$((currentScore - 1))
    fi
  # else
...
check_2_17() {
  ...
  if get_docker_effective_command_line_args '--no-new-privileges' | grep "no-new-privileges" >/dev/null 2>&1; then
    ...
    warn "$check_2_17"
    note "     * Yoy should run Docker daemon with: $ dockerd --no-new-privileges"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
  fi
}
