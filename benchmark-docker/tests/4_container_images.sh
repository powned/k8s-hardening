...
check_4_2() {
  ...
  totalChecks=$((totalChecks + 1))
  info "$check_4_2"
  note "     * Review the history of commits: $ docker history <image_name>"
  resulttestjson "NOTE"
  currentScore=$((currentScore + 0))
}
...
check_4_3() {
  ...
  totalChecks=$((totalChecks + 1))
  info "$check_4_3"
  note "     * Execute for each container for listing packages: $ docker exec <container_name> rpm -qa"
  resulttestjson "NOTE"
  currentScore=$((currentScore + 0))
}
...
check_4_5() {
  ...
  if [ "x$DOCKER_CONTENT_TRUST" = "x1" ]; then
    ...
    warn "$check_4_5"
    note "     * Enable contest trust with: $ export DOCKER_CONTENT_TRUST=1"
    note "     * Add to /etc/environment to make it a system permanent variable"
    resulttestjson "WARN"
    currentScore=$((currentScore - 1))
  fi
}
...
check_4_6() {
  ...
  for img in $images; do
    if docker inspect --format='{{.Config.Healthcheck}}' "$img" 2>/dev/null | grep -e "<nil>" >/dev/null 2>&1; then
      ...
      if ! [ "$imgName" = '[]' ]; then
        note "     * No HEALTHCHECK found: $imgName"
        no_health_images="$no_health_images $imgName"
      else
        note "     * No HEALTHCHECK found: $img"
...
check_4_8() {
  ...
  totalChecks=$((totalChecks + 1))
  info "$check_4_8"
  note "     * Allow setuid and setgui only on executables which require them."
  note "     * You could remove them adding at the end of your DockerFile"
  note "     * (CAUTION: modify the command to suit your requirements and to not reduce permissions of legitimate programs)"
  note "     * RUN find / -perm /6000 -type f -exec chmod a-s {} \; || true"
  resulttestjson "NOTE"
  currentScore=$((currentScore + 0))
}
